...
for(mm in 1:7){
  ...
  for (ll in 1:2){
    ...
    for(kk in 1:12){
      # load model
      sPath = file.path(dirPath,'res_knn',SNameS)
      mon = mons[mm]
  ...
  #targetpred_knn[[mm]] = scoresAll
  save(scoresAll, file = file.path(dirPath,'processing',paste0('targetpred_knn_sources_ex',mm,'.rData')))
}

save(targetpred_knn, file = file.path(dirPath,'processing','targetpred_knn_sources.rData'))
