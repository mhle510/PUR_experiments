...
for(mm in 1:7){
  ...
  for(kk in 1:12){
    ...
    # path create
    dir.create(file.path(dirPath,'res_knn'), showWarnings = F)
    dir.create(file.path(dirPath,'res_knn',SName),showWarnings = F)
    ...
    id = 1
    for(it in 1:length(kNums)){
        #cat(id, '\n')
        #make this example reproducible
        set.seed(1)
        #fit the random forest model
        model = knnreg(x = scaled_train1dt[,-ncol(scaled_train1dt)],
                       y = scaled_train1dt[,ncol(scaled_train1dt)],
                       k = kNums[it])
    ...
    kbest = kNums[metricsTest1$knum[bestloc]]
    
    # test with best model after trial and error
    set.seed(1)
    bestModel = knnreg(x = scaled_train1dt[,-ncol(scaled_train1dt)],
                       y = scaled_train1dt[,ncol(scaled_train1dt)],
                       k = kbest)
    ...
    for(ll in 1 : nn){
      ...
      metricsTestFinal$knum[ll] = kbest
    ...
    save(listOutputrf, metricsTestFinal, file = file.path(sPath,paste0('knn_mon',mon,'.rData')))
  }
}
