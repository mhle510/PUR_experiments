...
for(mm in 1:7){
  for(kk in 1:12){
    cat('--------------','\n')
    cat(SNames[mm],'---',dirmonth[kk],'\n')
    SName = SNames[mm]
    mon = mons[kk]
    proccessedPathS = paste(dirPath,'/','processing/',SName , '/', dirmonth[kk],sep = '')
    
    # path create
    dir.create(file.path(dirPath,'res_xgb'), showWarnings = F)
    dir.create(file.path(dirPath,'res_xgb',SName),showWarnings = F)
    sPath = file.path(dirPath,'res_xgb',SName)
    
    
    
    # step1 finding optimize of tree and split features -----------------------
    etaValues = seq(0.1,0.9,0.1)  # eta controls learning rate, in between 0 and 1
    gammaValues = seq(0.1, 15, 0.2) # pruning control
    #lamdaValues = seq(0.5,2,0.5) # loss estimatation control
    nn = length(etaValues) * length(gammaValues)
    ...
    metricsTest1 = data.frame(no = numeric(nn),
                              eta = numeric(nn), # learning rate
                              gamma = numeric(nn), # minimum loss reduction
    ...
    set.seed(1)
    id = 1
    for(ieta in 1:length(etaValues)){
      for(igam in 1: length(gammaValues)){
        #fit the xgboost model
        model = xgboost(
          data = data.matrix(scaled_train1dt[,-ncol(scaled_train1dt)]),
          label = scaled_train1dt$logQ,
          eta = etaValues[ieta],
          gamma = gammaValues[igam],
          nrounds = 100, # maximum iteration
          max_depth = 6, # maxim tree nodes
          eval_metric = "rmse", 
        ...
        logsimTrain1 = predict(model, data.matrix(scaled_train1dt[,-ncol(scaled_train1dt)]))
        logsimTrain2 = predict(model, data.matrix(scaled_train2dt[,-ncol(scaled_train2dt)]))
        logsimTest = predict(model, data.matrix(scaled_testdt[,-ncol(testdt)]))
    ...
    summary(kge12)
    plot(kge12)
    bestloc = which(kge12 == max(kge12, na.rm  =T))
    ...
    etabest = etaValues[metricsTest1$eta[bestloc]]
    gambest = gammaValues[metricsTest1$gamma[bestloc]]
    ...
    logsimTrain1 = predict(bestModel, data.matrix(scaled_train1dt[,-ncol(scaled_train1dt)]))
    logsimTrain2 = predict(bestModel, data.matrix(scaled_train2dt[,-ncol(scaled_train2dt)]))
    logsimTest = predict(bestModel, data.matrix(scaled_testdt[,-ncol(testdt)]))
    ...
    for(ll in 1 : nn){
      ...
      metricsTestFinal$eta[ll] = etabest
      metricsTestFinal$gamma[ll] = gambest
    ...
    save(listOutputrf, metricsTestFinal, file = file.path(sPath,paste0('xgb_mon',mon,'.rData')))
  }  
}
