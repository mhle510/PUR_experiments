...
for(mm in 1:7){
  ...
  for (ll in 1:2){
    ...
    for(kk in 1:12){
      # load model
      sPath = file.path(dirPath,'res_rf',SNameS)
      mon = mons[mm]
      ...
      for(ip in 1:100){
        ...
        # run and obtain simulation results from 100 models at sources
        for(im in 1:100){
          sourcemodel = listOutputrf[[im]]$model
          logsimTest = predict(sourcemodel, newdata=scaled_testdt[,-ncol(testdt)])
  ...
  #targetpred_rf[[mm]] = scoresAll
  save(scoresAll, file = file.path(dirPath,'processing',paste0('targetpred_rf_sources_ex',mm,'.rData')))
}

#save(targetpred_rf, file = file.path(dirPath,'processing','targetpred_rf_sources.rData'))
